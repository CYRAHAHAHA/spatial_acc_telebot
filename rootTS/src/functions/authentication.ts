...
export class AutodeskAuth {
    ...
    constructor(client_id: string, client_secret: string, redirect_uri: string, scopes: string) {
        ...
        if (process.env.RAILWAY_ENVIRONMENT) {
             ...
             this.token_file = path.join(dataDir, "autodesk_tokens.json");
        }
        this.load_tokens();
    }
    // ... (This is a 1:1 copy of the class in src/authentication.ts for the purpose of the requested structure)
    // Reuse logic from src/authentication.ts methods...
    save_tokens(access_token: string, refresh_token: string, expires_in: number) {
        ...
        fs.writeFileSync(this.token_file, JSON.stringify({ access_token, refresh_token, expires_at: this.expires_at }, null, 2));
    }
    load_tokens(): boolean {
    ...
    async exchange_code_for_tokens(code: string): Promise<string | null> {
        ...
        const data = querystring.stringify({ client_id: this.client_id, client_secret: this.client_secret, grant_type: "authorization_code", code, redirect_uri: this.redirect_uri });
        try {
    ...
    async refresh_access_token(): Promise<string | null> {
        ...
        const url = "https://developer.api.autodesk.com/authentication/v2/token";
        const data = querystring.stringify({ client_id: this.client_id, client_secret: this.client_secret, grant_type: "refresh_token", refresh_token: this.refresh_token });
        try {
            const res = await axios.post(url, data);
            this.save_tokens(res.data.access_token, res.data.refresh_token, res.data.expires_in);
            return this.access_token;
        } catch (e) { if (fs.existsSync(this.token_file)) fs.unlinkSync(this.token_file); return null; }
    }
    get_auth_url(): string {
