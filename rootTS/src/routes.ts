...
import {
  fetchIssueSubtypes,
  formatSubtypesOutput,
} from "./functions/fetchIssueSubtypes";
...
router.get(
  "/do_all_initial_setup",
  requireAccessToken(true),
  async (req: Request, res: Response) => {
    const token = (req as any).token;

    // --- FIX START: Proper Mock Response Object ---
    // This object mimics 'res' so the sub-functions don't crash or end the real request early.
    const mockRes = {
      redirect: (url: string) => {
        console.log(`[Setup Step Completed] Mock redirect to: ${url}`);
      },
      status: (code: number) => {
        console.log(`[Setup Step Error] Status Code: ${code}`);
        return {
          json: (body: any) => console.error(`[Setup Step Error Body]:`, body),
        };
      },
    ...
    try {
      console.log("--- 1. Fetching Assets Config ---");
      await fetchAssetsConfig(req, mockRes, token);

      console.log("--- 2. Fetching All Assets Info ---");
      await fetchAllAssetsInfo(mockRes, token);

      console.log("--- 3. Fetching IFC Metadata ---");
      await fetchIfcMetadata(mockRes, token);

      // Finally, perform the REAL redirect to the browser
      res.redirect(
        `/?msg=${encodeURIComponent(
          "All initial setup functions called successfully. Check server logs for details."
        )}`
      );
    } catch (e: any) {
      console.error("Critical error in setup chain:", e);
      res.redirect(
        `/?msg=${encodeURIComponent("Error during setup: " + e.message)}`
      );
    }
  }
);
...
router.get("/data/:filename", (req, res) => {
  res.sendFile(
    path.join(path.resolve(__dirname, "../data"), req.params.filename)
  );
});
...
router.post("/update_status", requireAccessToken(true), async (req, res) => {
  ...
  const token = (req as any).token;
  if (!status_value || !asset_guid)
    return res.status(400).json({ error: "Missing guid or status" });
...
router.post("/create_issue", requireAccessToken(true), async (req, res) => {
  const { title, status, issue_subtype_id, description, location_description } =
    req.body;
  const token = (req as any).token;
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (config.project_id) {
        try {
          const r = await axios.get(
            `https://developer.api.autodesk.com/construction/issues/v1/projects/${config.project_id}/issues`,
            {
      ...
      res.json({
        success: true,
        total_count: Object.keys(subtypes).length,
