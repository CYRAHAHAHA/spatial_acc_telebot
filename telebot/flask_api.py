...
def update_guid(guid: str):
    ...
    if status_value:
        try:
            ...
            print(f"ERROR: Failed to forward to /update_issue: {exc}")
            return jsonify({
                "ok": False,
    ...
    # No status provided: just confirm local logging
    return jsonify({
        "ok": True,
        "guid": guid,
        "version": DB[guid]["version"],
        "message": "Update logged locally (no status provided to forward)",
    }), 200


if __name__ == "__main__":
    print("Flask API starting. Writing to", SITE_UPDATES_PATH)
    port = int(os.getenv("TELEBOT_FLASK_PORT", 5000))
    print(f"Starting on port {port}, ROOT_APP_BASE={ROOT_APP_BASE}")
